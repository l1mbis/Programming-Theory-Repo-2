    ...
    private void OnTriggerEnter(Collider other) {
        if (other.gameObject.CompareTag("Normal_Fence") && !GameManager.Instance.gameOver) {
            if(other.transform.parent.gameObject.name == "Fence(Clone)")
                GameManager.Instance.updateScore(other.transform.parent.GetComponent<Fence_Normal>().ScoreToAdd());
            else
                GameManager.Instance.updateScore(other.transform.parent.GetComponent<Fence_Gold>().ScoreToAdd());
        }
    }
